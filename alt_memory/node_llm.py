...
PROMPT_TEMPLATE = """You are reading content filed in a memory dimension. Generate a
...
def _call_llm(cfg: LLMConfig, source_file: str, realm: str, domain: str, content: str):
    try:
        from alt_memory.i18n import t
...
def regenerate_nodes(
    ...
    if total == 0:
        print("No entities in dimension.")
        return {"processed": 0}
    ...
    offset = 0
    while offset < total:
        batch = entities_col.get(limit=batch_size, offset=offset, include=["documents", "metadatas"])
        ids = batch["ids"]
        if not ids:
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dimension",
        default=os.path.expanduser("~/.alt-memory"),
        help="Path to the dimension",
    )
    parser.add_argument("--realm", default=None, help="Limit to one realm")
