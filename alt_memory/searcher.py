...
def expand_with_neighbors(
    ...
    """Expand a matched entity with its *radius* sibling chunks in the same source file.
    ...
    try:
        cursor = db.execute(
            "SELECT id, realm, domain, content, metadata FROM entities WHERE source_file = ?",
            (src,),
        )
    ...
    return {
        "text": combined,
        "entity_index": chunk_idx,
        "total_entities": len(all_docs),
    }


# ── Virtual line numbering (read-time grid for entities) ─────────────────
