...
def _resolve_bert_embedder():
    ...
    from alt_memory.backends.bert_embedder import NumpyBertEmbedder
...
def get_embedder(
    ...
    ``cuda``, ``coreml``, ``dml``. Only applies to ONNX-based models.
    """
    if model == "numpy":
    ...
    if model == "sentence":
        ...
        ef = OnnxEmbedder(device=device)
    elif model == "embeddinggemma":
        providers, _ = resolve_embedding_device(device)
...
_ACCELERATOR_MAP: dict[str, tuple[str, str]] = {
    "cuda": ("CUDAExecutionProvider", "alt-memory[gpu]"),
    "coreml": ("CoreMLExecutionProvider", "alt-memory[coreml]"),
    "dml": ("DmlExecutionProvider", "alt-memory[dml]"),
}
...
class EmbeddinggemmaONNX:
    ...
    384-dim FAISS index via Matryoshka truncation.
    """
