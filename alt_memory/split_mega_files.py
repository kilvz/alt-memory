...
HOME = Path.home()
LUMI_DIR = Path(os.environ.get("ALT_MEMORY_SOURCE_DIR", str(HOME / "Desktop/transcripts")))

# People we know about (for name detection in content)
# Loaded from ~/.alt-memory/known_names.json if it exists, otherwise generic fallback.
_KNOWN_NAMES_PATH = HOME / ".alt-memory" / "known_names.json"
...
def extract_people(lines):
    ...
    if dir_match:
        ...
        # User can map usernames to names in ~/.alt-memory/known_names.json
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Source directory (default: ALT_MEMORY_SOURCE_DIR or ~/Desktop/transcripts)",
    )
    parser.add_argument(
