...
def _load_entity_names(dim_path: str = None) -> set:
    ...
    try:
        ...
        kg_path = os.path.join(dim_path or config.dim_path, "knowledge_graph.sqlite3")
...
def check_text(text: str, dim_path: str = None, config=None) -> list:
    if config is None:
        from alt_memory.config import AltMemoryConfig
        config = AltMemoryConfig()
    if dim_path is None:
    ...
    issues: list = []
    entity_names = _load_entity_names(dim_path)

    issues.extend(_check_entity_confusion(text, entity_names))
    issues.extend(_check_kg_contradictions(text, dim_path))
...
def _check_kg_contradictions(text: str, dim_path: str) -> list:
