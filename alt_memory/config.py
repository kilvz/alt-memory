"""alt-memory configuration system.

Priority: env vars > config file (~/.alt-memory/config.json) > defaults
"""
...
def normalize_realm_name(name: str) -> str:
...
DEFAULT_DIM_PATH = os.path.expanduser("~/.alt-memory")
DEFAULT_CHUNK_SIZE = 800
DEFAULT_CHUNK_OVERLAP = 100
DEFAULT_MIN_CHUNK_SIZE = 50

DEFAULT_TOPIC_REALMS = [
    "emotions",
    "consciousness",
    "memory",
    "technical",
...
class AltMemoryConfig:
    """Configuration manager for alt-memory.
    ...
    @property
    def dim_path(self):
        env_val = os.environ.get("ALT_MEMORY_PATH")
        if env_val:
            return os.path.abspath(os.path.expanduser(env_val))
        return self._file_config.get("dim_path", DEFAULT_DIM_PATH)
    ...
    def entity_languages(self):
        env_val = os.environ.get("ALT_ENTITY_LANGUAGES")
        if env_val:
    ...
    def topic_tunnel_min_count(self):
        env_val = os.environ.get("ALT_TOPIC_TUNNEL_MIN_COUNT")
        if env_val:
            try:
                parsed = int(env_val)
                if parsed >= 1:
    ...
    def hooks_auto_save(self) -> bool:
        val = os.environ.get("ALT_HOOKS_AUTO_SAVE")
        if val is not None:
    ...
    def hook_silent_save(self) -> bool:
        val = os.environ.get("ALT_HOOK_SILENT_SAVE")
        if val is not None:
            return val.lower() in ("true", "1", "yes", "on")
    ...
    def hook_desktop_toast(self) -> bool:
        val = os.environ.get("ALT_HOOK_DESKTOP_TOAST")
        if val is not None:
            return val.lower() in ("true", "1", "yes", "on")
    ...
    def default_embedder(self) -> str:
        """Default embedder model for new dimensions. One of: sentence, spacy, numpy, bert, minilm, embeddinggemma."""
        env_val = (os.environ.get("ALT_DEFAULT_EMBEDDER")
                   or os.environ.get("ALT_MEMORY_EMBEDDING_MODEL"))
        if env_val:
    ...
    def embedding_device(self) -> str:
        """ONNX device for embedding models: auto, cpu, cuda, coreml, dml.
        Reads ALT_MEMORY_EMBEDDING_DEVICE env var or config.json embedding_device."""
        env_val = os.environ.get("ALT_MEMORY_EMBEDDING_DEVICE")
        if env_val:
            return env_val.strip().lower()
    ...
    def embedding_model(self) -> str:
        """Embedding model: minilm or embeddinggemma.
        Reads ALT_MEMORY_EMBEDDING_MODEL env var or config.json embedding_model."""
        env_val = os.environ.get("ALT_MEMORY_EMBEDDING_MODEL")
        if env_val:
            return env_val.strip().lower()
    ...
    def people_map(self) -> dict:
        """Mapping of name variants to canonical names, from people_map.json."""
        people_file = self._config_dir / "people_map.json"
    ...
    def save_people_map(self, people_map: dict) -> dict:
        self._config_dir.mkdir(parents=True, exist_ok=True)
        people_file = self._config_dir / "people_map.json"
