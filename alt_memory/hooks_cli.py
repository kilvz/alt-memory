"""Hook logic for Alt Memory — session-start, stop, and precompact hooks for Claude Code / Codex."""
...
SAVE_INTERVAL = 15
STATE_DIR = Path.home() / ".alt-memory" / "hook_state"
DIM_ROOT = Path.home() / ".alt-memory"
...
def _dim_root_exists() -> bool:
    return DIM_ROOT.is_dir()


def _alt_memory_python() -> str:
    env_python = os.environ.get("ALT_MEMORY_PYTHON", "")
...
def _get_mine_targets() -> list[tuple[str, str]]:
    ...
    mempal_dir = os.environ.get("ALT_MEMORY_DIR") or os.environ.get("MEMPAL_DIR", "")
    if mempal_dir:
...
_MINE_PID_FILE_ENV = "ALT_MEMORY_MINE_PID_FILE"
_MINE_TIMEOUT_HOURS_ENV = "ALT_MEMORY_MINE_TIMEOUT_HOURS"
...
def _maybe_auto_ingest():
    ...
    for mine_dir, mode in targets:
        try:
            _spawn_mine([_alt_memory_python(), "-m", "alt_memory", "mine", mine_dir, "--mode", mode])
        except OSError:
            pass
...
def _mine_sync():
    ...
    for mine_dir, mode in targets:
        try:
            with open(log_path, "a") as log_f:
                subprocess.run(
                    [_alt_memory_python(), "-m", "alt_memory", "mine", mine_dir, "--mode", mode],
...
def _desktop_toast(body: str, title: str = "Alt Memory"):
    try:
        subprocess.Popen(
            ["notify-send", "--app-name", "Alt Memory", "--icon=brain", title, body],
...
def _realm_from_jsonl_cwd(transcript_path: str) -> Optional[str]:
    try:
        path = Path(transcript_path).expanduser()
        if not path.is_file():
            return None
        with path.open("r", encoding="utf-8", errors="replace") as f:
            for i, line in enumerate(f):
                ...
                if project:
                    ...
                    return f"realm_{slug}"
    except OSError:
        pass
    return None


def _realm_from_transcript_path(transcript_path: str) -> str:
    cwd_realm = _realm_from_jsonl_cwd(transcript_path)
    if cwd_realm:
        return cwd_realm
...
def hook_stop(data: dict, harness: str):
    ...
    if not AltMemoryConfig().hooks_auto_save:
        _output({})
        return
    ...
    if str(stop_hook_active).lower() in ("true", "1", "yes"):
        silent_guard = True
        try:
            silent_guard = AltMemoryConfig().hook_silent_save
    ...
    if since_last >= SAVE_INTERVAL and exchange_count > 0:
        ...
        project_realm = _realm_from_transcript_path(transcript_path)
        ...
        if silent:
            ...
            if count > 0:
                ...
                _output({"systemMessage": f"✦ {count} memories woven into the dimension{tag}"})
            else:
                _output({})
        else:
            try:
